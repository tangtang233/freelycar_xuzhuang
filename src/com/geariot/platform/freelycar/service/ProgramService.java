...
public class ProgramService {
    ...
    public String addProgram(Program program) {
        ...
        if (exist != null) {
            return JsonResFactory.buildOrg(RESCODE.DUPLICATED_ERROR).toString();
        } else {
    ...
    /**
     * 修改项目类别
     * @param program 项目类别对象
     * @return jsonString
     */
    public String modifyProgram(Program program) {
        Program exist = programDao.findProgramByName(program.getName());
        if (exist != null && exist.getId() != program.getId()) {
            return JsonResFactory.buildOrg(RESCODE.DUPLICATED_ERROR).toString();
        }

        Program pro = programDao.findProgramByProgramId(program.getId());
        pro.setName(program.getName());
        pro.setComment(program.getComment());

        programDao.update(pro);
        JsonConfig config = JsonResFactory.dateConfig();
        return JsonResFactory.buildNetWithData(RESCODE.SUCCESS, net.sf.json.JSONObject.fromObject(pro, config)).toString();
    }
