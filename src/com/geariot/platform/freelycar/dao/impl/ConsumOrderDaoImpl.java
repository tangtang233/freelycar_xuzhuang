...
public class ConsumOrderDaoImpl implements ConsumOrderDao {
    ...
    public List<OrderSummary> listAllPaidOrders(String startTime, String endTime, int from, int pageSize) {
        String sql = "SELECT co.id, co.clientId, co.carBrand, co.licensePlate, c.`name`, c.phone, co.actualPrice + co.actualPrice1 AS totalActualPrice, co.totalPrice, pn.projectName, co.createDate, c.isMember FROM consumorder co LEFT JOIN client c ON c.id = co.clientId LEFT JOIN ( SELECT pi.consumOrderId, GROUP_CONCAT( pi.NAME ) AS projectName FROM projectinfo pi GROUP BY pi.consumOrderId ) AS pn ON pn.consumOrderId = co.id WHERE co.createDate > :startTime AND co.createDate < :endTime AND co.payState = 1 ORDER BY co.createDate DESC";
        Query query = this.getSession().createSQLQuery(sql).setString("startTime", startTime).setString("endTime", endTime).setFirstResult(from).setMaxResults(pageSize).setCacheable(Constants.SELECT_CACHE);
    ...
    public long getAllPaidOrdersCount(String startTime, String endTime) {
        String sql = "SELECT count(1) as num FROM consumorder co LEFT JOIN client c ON c.id = co.clientId LEFT JOIN ( SELECT pi.consumOrderId, GROUP_CONCAT( pi.NAME ) AS projectName FROM projectinfo pi GROUP BY pi.consumOrderId ) AS pn ON pn.consumOrderId = co.id WHERE co.createDate > :startTime AND co.createDate < :endTime AND co.payState = 1 ORDER BY co.createDate DESC";
        Query query = this.getSession().createSQLQuery(sql).setString("startTime", startTime).setString("endTime", endTime).setCacheable(Constants.SELECT_CACHE);
        query.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP);
